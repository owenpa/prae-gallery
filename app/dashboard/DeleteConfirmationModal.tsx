...
export default function DeleteConfirmationModal (): JSX.Element {
    ...
    <dialog id='delete-confirmation-modal'>
      <div className='border px-4 py-6 rounded-lg tracking-wide max-h-[300px] max-w-xl gap-2'>
        <div className='flex items-center justify-between'>
          <h1 id="delete-display-image-name" className="text-xl font-semibold inline"></h1>
          <button onClick={() => { (document.getElementById('delete-confirmation-modal') as HTMLDialogElement).close() }}><X className='self-end justify-self-end'/></button>
        </div>
        <form action={fireDeleteImage} className='gap-2 flex flex-col'>
          ...
          <div className='flex gap-2'>
            <Input required name="image-name" id="image-name" onInput={(e) => { (e.target as HTMLInputElement).value === document.getElementById('delete-display-image-name')?.innerText ? setCanSubmit(true) : setCanSubmit(false) }}></Input>
